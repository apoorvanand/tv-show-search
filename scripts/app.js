(function(){
    ...
    function displayShowResults(results){  
       $('#tableBody').html('<td id="episodeName"  data-episodeid= "' + results.id + '">' + results.name + '</td>' + '<td>' + checkNetwork(results.network)+ '</td>' + '<td>' + results.type + '</td>' + '<td> <img  class="image-border" src= " ' + results.image.medium+ '"> </td>'+'<td>'+'Status : '+results.status+ '<br>'+'<a href='+results.officialSite+``+'>'+'Official link'+'</a>'+'<br>'+'Genre : '+results.genres+'<br>'+'Schedule : '+results.schedule.days+' , '+results.schedule.time+'</td>');
